...
imputed <-  missForest(data, 10, ntree = 500, verbose = TRUE)
...
impvar <- rownames(imprf1)[order(imprf1[, 1], decreasing=TRUE)]
op <- par(mfrow=c(2, 3))

for (i in seq_along(impvar)) {
    partialPlot(rf1,  imputed_full, impvar[i], xlab=impvar[i],
...
```
----------------------------------------------------------------------
# Running RF again after tuning mtry = 8
----------------------------------------------------------------------
```{r}
